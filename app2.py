...
def main():
    ...
    if user_input:
        ...
        with st.spinner("Processing your query..."):
            ...
            else:  # Hybrid Simple + Graph
                data = hybrid_simple_graph_search(user_input)

            if not data:
                st.info("Δεν βρέθηκαν ακριβείς πληροφορίες. Ίσως εννοείτε:")
                suggestions = suggest_nodes(user_input, ALL_NODES, limit=10)
                for s in suggestions:
                    st.button(
                        label=s,
                        key=f"suggest_{s}",
                        on_click=paste_suggestion_to_buffer,
                        args=(s,)
                    )
